...
- 자동 폴링을 사용하지 않는다. 앱 진입 시 1회 로드하고, 이후에는 사용자가 `상태 새로고침` 또는 저장/선택/로그인 같은 명시적 액션을 수행할 때만 `/api/agenda`를 호출한다.
...
- 공유 보드판 자동 동기화는 아직 구현하지 않는다. 이 단계의 목표는 개인 보유물과 비밀 아젠다 확인이다.
